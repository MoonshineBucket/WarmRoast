...
public class StackTraceNode extends StackNode {
    ...
    public String getNameHtml(McpMapping mapping) {
        ...
        if(classMapping != null) {
            String className = "<span class=\"matched\" title=\"" + escapeHtml(getClassName()) +
                    "\">" + escapeHtml(classMapping.getActual()) + "</span>";
            List<String> actualMethods = classMapping.mapMethod(getMethodName());

            if(actualMethods.size() == 0) return className + "." + escapeHtml(getMethodName()) + "()";
            else if (actualMethods.size() == 1) {
                return className + ".<span class=\"matched\" title=\"" +
                        escapeHtml(getMethodName()) + "\">" + escapeHtml(actualMethods
                        .get(0)) + "</span>()";
            } else {
                ...
                return className + ".<span class=\"multiple-matches\" title=\"" + builder.toString() +
                        "\">" + escapeHtml(getMethodName()) + "</span>()";
            }
        } else {
            String actualMethod = mapping.mapMethodId(getMethodName());
            if(actualMethod == null) return escapeHtml(getClassName()) + "." +
                    escapeHtml(getMethodName()) + "()";
            else {
                return className + ".<span class=\"matched\" title=\"" + escapeHtml(getMethodName()) +
    ...
    public int compareTo(StackNode o) {
        if(getTotalTime() == o.getTotalTime()) return 0;
        else if(getTotalTime()> o.getTotalTime()) return -1;
        else return 1;
    }
